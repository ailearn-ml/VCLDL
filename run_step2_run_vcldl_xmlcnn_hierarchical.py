...
from methods.vcldl_hierarchical import HierarchicalVCLDL
...
def main_train():
    ...
    model = HierarchicalVCLDL(num_labels=y_train.shape[1],
                              fixed=fixed,
...
method_name = 'hierarchical-vcldl-xmlcnn'
...
params = json.load(open(
    os.path.join(os.getcwd(), './configs', method_name.replace('hierarchical-vcldl-', ''),
                 dataset_name + '.json')))
max_epoch = 10
num_workers = 8
