...
ACT2FN = {'elu': F.elu, 'relu': F.relu, 'sigmoid': torch.sigmoid, 'swish': swish,
          'tanh': torch.tanh, 'leakyrelu': nn.LeakyReLU()}
...
class CorNet(nn.Module):
    def __init__(self, output_size, cornet_dim=200, n_cornet_blocks=1, **kwargs):
...
class VariationalLDL(nn.Module):
    def __init__(self, n_labels):
        super(VariationalLDL, self).__init__()
        self.n_labels = n_labels
        self.encoder_x = CorNet(n_labels, n_cornet_blocks=1)
        self.encoder_y = CorNet(n_labels, n_cornet_blocks=1)
        self.mlp = CorNet(n_labels, n_cornet_blocks=1)
        self.linear1 = CorNet(n_labels, n_cornet_blocks=1)
        self.linear2 = CorNet(n_labels, n_cornet_blocks=1)
    ...
    def vcldl_forward(self, x, y):
        ...
        d = self.mlp(x_encode + y_encode)
        mu = self.linear1(d)
        var = F.softplus(self.linear2(d)) + 1e-8
        rand = torch.normal(mean=0., std=1., size=mu.shape).cuda()
        z = mu + (var ** 0.5) * rand
        x_decode = self.decoder_x(z)
        y_decode = self.decoder_y(x_decode)
        return x_encode, mu, var, x_decode, y_decode


class VCLDL(nn.Module):
    def __init__(self, num_labels, basic_model, fixed=True,
                 alpha=0.1, lr=1e-3, weight_decay=1e-5, val_frequence=100):
        super(VCLDL, self).__init__()
        self.alpha = alpha
        self.fixed = fixed
        self.basic_model = basic_model
        self.VariationalLDL = VariationalLDL(num_labels)
    ...
    def set_forward_loss(self, batch):
        ...
        x_encode, mu, var, x_decode, y_decode = self.VariationalLDL.vcldl_forward(x, y)
        loss_cls = nn.BCEWithLogitsLoss()(self.VariationalLDL(x), y)
        loss_tgt = 0.5 * nn.MSELoss()(x_encode, x_decode)
        loss_dec = nn.BCEWithLogitsLoss()(y_decode, y)
        pm, pv = torch.zeros(mu.shape).cuda(), torch.ones(var.shape).cuda()
        loss_var = kl_normal(mu, var, pm, pv).mean()
